...
/*   Updated: 2022/07/26 17:20:38 by jihoh            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_cam(t_cam *cam)
{
	...
	cam->focal_len = 1.0;
	cam->origin = create_vec3(0, 0, 0);
	cam->horizontal = create_vec3(cam->vport_w, 0, 0);
	cam->vertical = create_vec3(0, cam->vport_h, 0);
	cam->l_l_c = vsub(cam->origin, vdiv_n(cam->horizontal, 2));
	cam->l_l_c = vsub(cam->l_l_c, vdiv_n(cam->vertical, 2));
	cam->l_l_c = vsub(cam->l_l_c, create_vec3(0, 0, cam->focal_len));
}
